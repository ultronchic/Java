...
public class Main {
    ...
    class Main {
        public static void main(String[] args) {
            edu.mfti.FractionCache fractionCache = new edu.mfti.FractionCache();

            // Получаем дробь 1/2
            edu.mfti.Fractionable fraction1 = fractionCache.getFraction(1, 2);
            System.out.println("Fraction 1: " + fraction1); // Output: 1/2

            // Получаем дробь 1/2 снова (должен использоваться кэш)
            edu.mfti.Fractionable fraction2 = fractionCache.getFraction(1, 2);
            System.out.println("Fraction 2: " + fraction2); // Output: 1/2

            // Проверяем, что оба объекта ссылаются на один и тот же экземпляр
            System.out.println("Are both fractions the same instance? " + (fraction1 == fraction2)); // Output: true

            // Получаем другую дробь 2/3
            edu.mfti.Fractionable fraction3 = fractionCache.getFraction(2, 3);
            System.out.println("Fraction 3: " + fraction3); // Output: 2/3
        }
    }
// MainApp.java Задача 9.2.2 Отзывы. Создайте класс Отзыв, состоящий из текста отзыва в виде строки и целочисленной оценки.
//Далее создайте три бина типа Отзыв со следующими значениями:
//1.	Текст: “Очень хорошо”. Оценка: 4
//2.	Текст: “Сойдет”. Оценка:3
//3.	Текст: “Сложно сказать”. Оценка устанавливается бином random задачи 9.2.1.
...
import ru.Balakireva.people.Student;
            ...
            public class MainApp {
                ...
                public static void main(String[] args) {
                    ...
                    // Получаем студентов из контекста Spring
                    Student student1 = context.getBean("student1", Student.class);
                    Student student2 = context.getBean("student2", Student.class);

                    // Выводим информацию о студентах на экран
                    ...
                    if (comparisonResult > 0) {
                        System.out.println(student1.getName() + " имеет более высокую среднюю оценку.");
                    } else if (comparisonResult < 0) {
                        System.out.println(student2.getName() + " имеет более высокую среднюю оценку.");
                    } else {
                        System.out.println("Оба студента имеют одинаковую среднюю оценку.");
                    }

                    // студента с некорректной оценкой (вызывает исключение)
                    try {
                        Student invalidStudent = new Student("Некорректный Студент", 6.0, context.getBean("inRange", Predicate.class));
...
import org.springframework.context.annotation.AnnotationConfigApplicationContext;
import ru.Balakireva.people.Student;
import ru.Balakireva.people.StudentBuilder;
...
import java.util.Arrays;
            ...
            public class MainApp {
                ...
                public static void main(String[] args) {
                    ...
                    // Получаем построитель студентов из контекста Spring
                    StudentBuilder studentBuilder = context.getBean(StudentBuilder.class);

                    // Создаем студентов с указанием оценок и без них
                    Student student1 = studentBuilder.createStudent("Иванов Иван", Arrays.asList(3, 4, 5));
                    System.out.println(student1);

                    Student student2 = studentBuilder.createStudent("Петров Петр");
                    System.out.println(student2);

                    // Пример создания студента с некорректной оценкой (вызывает исключение)
                    try {
                        Student invalidStudent = studentBuilder.createStudent("Некорректный Студент", Arrays.asList(6));
                        System.out.println(invalidStudent);
                    } catch (IllegalArgumentException e) {
                        System.out.println(e.getMessage());
                    }
...
// MainApp.java 9.2.6 Стриминг платформа.
package ru.Balakireva.app;
...
import org.springframework.context.annotation.AnnotationConfigApplicationContext;
        ...
        public class MainApp {
            ...
            public static void main(String[] args) {
                ...
                // Получаем бин Person из контекста Spring.
                Person person = context.getBean(Person.class);

                // Выводим информацию о человеке.
                System.out.println(person); // Ожидается вывод: Person{name='vasia'}
            }
        }
...
import ru.Balakireva.models.Person;
        ...
        public class MainApp {
            ...
            public static void main(String[] args) {
                ...
                // Выводим информацию о человеке.
                System.out.println(person); // Ожидается вывод: Person{name='John Doe'}
            }
        }
//Задача 9.3.4
package ru.Balakireva.main;
import ru.Balakireva.models.Human;
import ru.Balakireva.others.ValidateException;
import ru.Balakireva.others.Validator;
        public class Main {
            public static void main(String[] args) {
                try {
...
import org.springframework.context.annotation.AnnotationConfigApplicationContext;
        ...
        public class MainApp {
            ...
            public static void main(String[] args) {
                ...
                // Проверяем наличие бина randomNumber в контейнере
                if (context.containsBean("randomNumber")) {
                    System.out.println("Бин randomNumber присутствует в контейнере.");
                    RandomNumber randomNumber = context.getBean(RandomNumber.class);

                    // Запросим несколько случайных чисел и выведем их значения
                    for (int i = 0; i < 5; i++) {
                        int randomValue = randomNumber.getRandom();
                        System.out.println("Случайное число: " + randomValue);
                    }
                } else {
                    System.out.println("Бин randomNumber отсутствует в контейнере.");
                }
            }
        }
//задача дз 20
public class Main {
    public static void main(String[] args) {
        DAO dao = new DAO("jdbc:mysql://localhost:3306/mydatabase", "username", "password");
